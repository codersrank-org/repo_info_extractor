...
def obfuscate(r):
    for i in range(len(r.commits)):
        o_user, o_email = _obfuscate_user(r.commits[i].author_name, r.commits[i].author_email)
        r.commits[i].author_name = o_user
        r.commits[i].author_email = o_email
        for n in range(len(r.commits[i].changed_files)):
            r.commits[i].changed_files[n].file_name = _obfuscate_filename(r.commits[i].changed_files[n].file_name)
